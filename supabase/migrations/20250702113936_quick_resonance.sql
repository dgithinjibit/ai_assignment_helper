/*
  # Complete AI Assignment Helper Schema

  1. New Tables
    - `profiles` - Extended user profiles with learning preferences
...
DROP POLICY IF EXISTS "Users can view own ai_results" ON ai_results;
DROP POLICY IF EXISTS "Users can view own competency_assessments" ON competency_assessments;
DROP POLICY IF EXISTS "Users can insert own competency_assessments" ON competency_assessments;
...
DROP POLICY IF EXISTS "Users can view own socratic_questions" ON socratic_questions;
...
-- Insert sample workflow templates (only if they don't exist)
...
SELECT * FROM (VALUES
  (
    ...
    '["content_extraction", "competency_mapping", "cbc_alignment", "socratic_generation", "feedback_creation", "learning_path_generation"]'::jsonb,
    '{"competency_analysis": {}, "socratic_questions": [], "feedback_sections": [], "learning_path": []}'::jsonb,
    '{"core_competencies": ["communication", "critical_thinking", "creativity"], "values": ["respect", "responsibility", "honesty"], "assessment_methods": ["formative", "authentic"]}'::jsonb
  ),
  (
    ...
    '["problem_identification", "solution_analysis", "reasoning_evaluation", "competency_assessment", "improvement_suggestions"]'::jsonb,
    '{"competency_analysis": {}, "socratic_questions": [], "feedback_sections": [], "learning_path": []}'::jsonb,
    '{"core_competencies": ["critical_thinking", "problem_solving", "communication"], "strands": ["number", "algebra", "geometry", "statistics"], "assessment_focus": ["understanding", "application", "reasoning"]}'::jsonb
  )
...
-- Insert sample CBC resources (only if they don't exist)
